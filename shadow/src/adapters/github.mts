...
async function awaitRun(opts: {
  ...
  for (let i = 0; i < attempts; i++) {
    const { status, conclusion } = JSON.parse(
      await capture(
        "gh",
        ["run", "view", String(opts.runId), "-R", opts.repo, "--json", "status,conclusion"],
        { env },
      ),
    ...
    if (state === "failure")
      throw new Error(`${opts.label} #${opts.runId} ${conclusion ?? "failed"}`);
    await delay(intervalMs);
  }
  throw new Error(
    `timed out waiting for ${opts.label} #${opts.runId} after ${(attempts * intervalMs) / 1000}s`,
  );
}
...
export async function dispatchReceiver(opts: {
  ...
  const out = await capture(
    ...
    [
      ...
      "-X",
      "POST",
      `repos/${opts.runnerRepo}/actions/workflows/${RECEIVER_WORKFLOW}/dispatches`,
      "--input",
      "-",
    ],
...
export async function watchRun(opts: {
  runnerRepo: string;
  runId: number;
  token: string;
}): Promise<void> {
  await awaitRun({
    repo: opts.runnerRepo,
    runId: opts.runId,
    token: opts.token,
    label: "receiver run",
  });
...
export async function findPrUrl(opts: {
  repo: string;
  branch: string;
  token: string;
}): Promise<string | null> {
...
export async function watchCommitRun(opts: {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    if (i === attempts - 1) {
      throw new Error(
        `no workflow run appeared for ${opts.repo}@${opts.sha} after ${(attempts * intervalMs) / 1000}s`,
      );
    }
    await delay(intervalMs);
  }
...
export async function closePrAndDeleteBranch(opts: {
  repo: string;
  branch: string;
  token: string;
}): Promise<void> {
  ...
  try {
    await capture(
      "gh",
      ["api", "-X", "DELETE", `repos/${opts.repo}/git/refs/heads/${opts.branch}`],
      { env },
    );
  } catch {

...
function main(): void {
  ...
  if (summaryFile) {
    const list = renderShadowList({
      consumers,
      workflowsPr: Number(args["workflows-pr"]),
      runnerRepo: args["runner-repo"],
    });
    appendFileSync(summaryFile, list);
  }

  console.log(
    `✅ ${consumers.length} consumer(s): ${consumers.map((c) => `${c.repo}@${c.ref}`).join(", ") || "(none)"}`,
  );
}

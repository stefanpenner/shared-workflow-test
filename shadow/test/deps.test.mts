...
const allow = { deps: ['yaml'], devDeps: ['typescript', '@types/node'] };

describe('disallowedDeps', () => {
  it('passes when only allowed deps are present', () => {
    assert.deepEqual(disallowedDeps({ dependencies: { yaml: '^2' }, devDependencies: { typescript: '^5' } }, allow), []);
  });

  it('flags a disallowed runtime dependency', () => {
    assert.deepEqual(disallowedDeps({ dependencies: { yaml: '^2', lodash: '^4' } }, allow), ['lodash']);
  });

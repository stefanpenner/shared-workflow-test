...
describe("resolveContext", () => {
  ...
  it("looks up the head SHA on workflow_dispatch", async () => {
    const ctx = await resolveContext({
      ...
      inputPr: "9",
      lookupHeadSha: async (pr) => `sha-of-${pr}`,
    });
  ...
  it("throws when a pull_request is missing its SHA", async () => {
    await assert.rejects(
      () => resolveContext({ eventName: "pull_request", prNumber: "7", lookupHeadSha: noLookup }),
      /head-sha/,
    );
  });
  ...
  it("throws when a dispatch is missing its PR", async () => {
    await assert.rejects(
      () =>
        resolveContext({ eventName: "workflow_dispatch", inputPr: "", lookupHeadSha: noLookup }),
      /input-pr/,
    );
  });
});

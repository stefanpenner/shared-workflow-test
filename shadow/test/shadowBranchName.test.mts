...
describe("shadowBranchName", () => {
  it("builds shadow/pr-<n>-<slug> from the consumer repo", () => {
    assert.equal(
      shadowBranchName({
        prNumber: 7,
        consumerRepo: "stefanpenner-cs/reusable-workflows-consumer",
      }),
  ...
  it("slugifies dots and the owner/name separator", () => {
    assert.equal(
      shadowBranchName({ prNumber: 1, consumerRepo: "org/lcc.live" }),
  ...
  it("lowercases", () => {
    assert.equal(
      shadowBranchName({ prNumber: 2, consumerRepo: "Org/MyRepo" }),
